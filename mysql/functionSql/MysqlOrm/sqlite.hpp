...
namespace ormpp
{
  ...
  public:
    sqlite() = default;
    // sqlite() { std::cout << "sqlite() " << std::endl; };
    // sqlite(std::string &&db_name) {
    //   std::string db_path = std::string("/tmp/") + db_name;
    //   if (fileExists(db_path))
    //     std::cout << db_path << " exsit " << std::endl;
    //   else
    //     std::cout << db_path << " nonexsit " << std::endl;
    // }
    ~sqlite() { disconnect(); }
    ...
    bool delete_records(Args &&...where_conditon)
    {
      ...
      std::cout << "delete_records: " << sql << std::endl;
      return true;
    }
        ...
        for_each0(
             ...
             this](auto item)
            {
              ...
              else if constexpr (std::is_same_v<decltype(item), ormpp_auto_key>)
              {
                ...
                append(sql, " INTEGER PRIMARY KEY AUTOINCREMENT");
